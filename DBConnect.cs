...
namespace GestaoFormandos
{
    internal class DBConnect
    {
        ...
        public bool DeleteFormador(string id)
        {
            ...
            try
            {
                if (OpenConnection())
                {
                    ...
                    {
                        // Adiciona o parâmetro ID à consulta
        ...
        {
            // Define a consulta SQL para excluir o registro da tabela "area" com o id_area específico passado como parâmetro
            ...
            try
            {
                if (OpenConnection())
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        dgv.Rows[idxLinha].Cells[0].Value = dr[0].ToString();// Define o valor da célula "ID" com o valor da primeira coluna da linha atual do MySqlDataReader
                        dgv.Rows[idxLinha].Cells[1].Value = dr[1].ToString();// Define o valor da célula "ALF2" com o valor da coluna "ALF2" da linha atual do MySqlDataReader
                        dgv.Rows[idxLinha].Cells[2].Value = dr[2].ToString();// Define o valor da célula "Nacionalidade" com o valor da coluna "Nacionalidade" da linha atual do MySqlDataReader
        ...
        public void PreencherDataGridViewFormador(ref DataGridView dgv, string id_formador, string nome, string nif, string dataNascimento, string id_area, string id_utilizador)
        {
        ...
        public void PreencherDataGriedViewFormandos(ref DataGridView dgv, string id_formando, string nome, string iban, string nacionalidade, string id_nacionalidade, char genero)
        {
            string query = "select id_formando, nome, iban, nacionalidade, id_nacionalidade, sexo from vFormando ";
            ...
            // Verifica se o parâmetro 'id_formando' tem algum valor e se algum filtro já foi aplicado
            if (id_formando.Length > 0 && flag == true)
            {
                // Se sim, adiciona cláusula AND para filtrar por nacionalidade
                query = query + " and id_formando = '" + id_formando + "'";
            }
            else if (id_formando.Length > 0)
            {
                // Se não, adiciona cláusula WHERE para filtrar por nacionalidade
                query = query + " where id_formando = '" + id_formando + "'";
            }
            ...
            if (iban.Length > 0 && flag == true)
            {
                // Se sim, adiciona cláusula AND para filtrar por iban
                query = query + " and iban = '" + iban + "'";
            }
            else if (iban.Length > 0)
            {
                // Se não, adiciona cláusula WHERE para filtrar por nacionalidade
                query = query + " where iban = '" + iban + "'";
            }
            if (nacionalidade.Length > 0 && flag == true)
            {
                // Se sim, adiciona cláusula AND para filtrar por iban
                query = query + " and nacionalidade = '" + nacionalidade + "'";
            }
            else if (nacionalidade.Length > 0)
            {
                // Se não, adiciona cláusula WHERE para filtrar por nacionalidade
                query = query + " where nacionalidade = '" + nacionalidade + "'";
            }
            // Adiciona a cláusula ORDER BY para ordenar os resultados por nome
        ...
        public bool DeleteNacionalidade(string id_nacionalidade)
        {
            ...
            try
            {
                if (OpenConnection())
                {
                    ...
                    // Verifica se houve linhas afetadas (ou seja, se a exclusão ocorreu com sucesso)
                    if (rowsAffected > 0)
                    {
                        success = true;
                    }
            ...
            {
                MessageBox.Show("Erro ao excluir a nacionalidade: " + ex.Message);
            }
            finally
            {
                CloseConnection();
